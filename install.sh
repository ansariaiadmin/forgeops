...
echo -e "${CYAN}"
cat <<'BANNER'
ForgeOps — v3.1.0 — تاریکی روشن شد — پشتیبانی صفر
کنترل پنل DevOps + RAG + Docker + JWT + WS logs + ناتیف — تاریکی روشن شد
AI RAG + Email + SMS + Telegram گروه
BANNER
echo -e "${NC}"
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}  🧙‍♂️ جادوگر نصب ForgeOps v3.1.0 — تاریکی روشن شد${NC}"
echo -e "${BLUE}  AI RAG + Email + SMS + Telegram گروه${NC}"
echo -e "${BLUE}========================================${NC}"
echo ""
echo -e "${YELLOW}سلام! 👋 کنترل پنل DevOps + RAG + Docker + JWT + WS logs + ناتیف — تاریکی روشن شد${NC}"
...
if [ "forgeops" = "legal-platform" ]; then
...
if [ "forgeops" = "universal-document-os" ]; then
...
if [ "$TELEGRAM_ENABLED" = "yes" ]; then
  ...
  if [ -n "$TELEGRAM_BOT_TOKEN" ] && command -v curl &> /dev/null; then
    ...
    if [ -n "$TELEGRAM_CHAT_ID" ]; then
      curl -sf -X POST -H "Content-Type: application/json" -d "{\"chat_id\":\"$TELEGRAM_CHAT_ID\",\"text\":\"🧪 تست ForgeOps v3.1.0 — تاریکی روشن شد — $(date)\"}" https://api.telegram.org/bot$TELEGRAM_BOT_TOKEN/sendMessage -o /dev/null 2>&1 && ok "Telegram پیام تست فرستاده شد — چک کن — تاریکی روشن شد" || err "Telegram پیام fail — Chat ID چک کن — ربات استارت کردی؟ — تاریکی روشن شد"
    fi
  fi
...
if [ "$SKIP_ENV" = "true" ]; then
  ...
  cat > .env <<EOF
# ForgeOps — .env — v3.1.0 — تاریکی روشن شد — $(date)
...
# AI — RAG — با هزینه — تاریکی روشن شد
...
ADMIN_EMAIL=admin@forgeops.dev
...
echo ""
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}  🎉 جادو تمام! ForgeOps آماده — تاریکی روشن شد! 🎉${NC}"
echo -e "${GREEN}========================================${NC}"
echo ""
echo -e "${BOLD}${BLUE}📍 دسترسی:${NC}"
echo -e "${GREEN}  🌐 http://localhost:3000 — کنترل پنل DevOps + RAG + Docker + JWT + WS logs + ناتیف — تاریکی روشن شد${NC}"
