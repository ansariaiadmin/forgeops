...
curl -X POST http://localhost:3000/api/auth/login \
  ...
  -d '{"email":"admin@forgeops.dev","password":"Admin@123"}'
```
...
{
  "token": "jwt-token-...",
  "user": { "email": "admin@forgeops.dev", "role": "admin" }
}
```
...
```json
{
  ...
  "providers": {
    ...
    "telegram": { "status": "ok", "bot": "@forgeops_notif_bot" }
  },
  "notifications": {
...
1. تلگرام → @BotFather → /newbot → اسم → یوزرنیم (مثل forgeops_notif_bot)
