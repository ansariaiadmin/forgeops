# 🧙‍♂️ جادوگر نصب ForgeOps — کنترل پنل DevOps — فوق ساده — پشتیبانی صفر — Zero Support
...
## 🎯 ForgeOps — کنترل پنل DevOps چیه؟ به زبان ساده:

پروژه‌ها + ایجنت‌ها + RAG + Docker Services + MCP + RBAC + audit logs + JWT refresh + WS logs streaming زنده
...
### 🤖 AI Provider — برای RAG (اختیاری)

**چیه؟** هوش مصنوعی برای جستجوی کد — RAG
...
**چیه؟** وقتی پروژه خطا می‌خوره یا دیپلوی می‌شه ناتیف می‌ده
...
## 🔔 سیستم ناتیفیکیشن — DevOps Alerts — سقف 10/10

**کانال‌ها:**
1. **In-App** — همیشه روشن
2. **Email** — خطا یا دیپلوی
3. **Telegram** — برای تیم DevOps — می‌تونه گروه باشه — Chat ID گروه

**ربات تلگرام گروه:** @BotFather → /newbot → توکن → ربات رو به گروه اضافه کن → https://api.telegram.org/bot<TOKEN>/getUpdates → chat_id گروه (با - شروع می‌شه)

**کد:** `lib/notification/service.ts` — سقف
...
**پشتیبانی صفر:** همه راهنماها همون‌جا بود — اگر بازم گیر کردی: https://github.com/ansariaiadmin/forgeops/issues
